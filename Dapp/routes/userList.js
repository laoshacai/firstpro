...
router.get('/', function (req, res, next) {
    ...
    var sql = 'select * from goin order by id desc';
    connection.query(sql, function (err, result, fields) {
        ...
        if (err) {
            ...
            res.render('userList', { data: resultList, pager: pager });
        }
    });
});
router.get('/:id', function (req, res, next) {
    var sql = 'delete from goin where id=?';
...
router.post('/', (req, res) => {
    var sql = 'select * from goin where phone=? or password=?';
    connection.query(sql, [req.body.searone, req.body.searone], function (err, result, fields) {
        ...
        if (err) {
            console.log('err', err);
            return;
        } else { res.render('userList', { data: resultList, pager: pager }); }
    });
});
